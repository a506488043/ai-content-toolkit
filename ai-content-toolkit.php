 ...
 * Description: 集成AI文章优化、SEO分析、网站卡片等10大功能的智能工具包，让网站内容管理更智能。
 * Version: 1.0.6
 * Author: chenghoufeng
 * Author URI: https://github.com/a506488043
 * License: GPL v2 or later
 * License URI: https://www.gnu.org/licenses/gpl-2.0.html
 * Text Domain: ai-content-toolkit
...
// 定义插件常量
define('AI_CONTENT_TOOLKIT_VERSION', '1.0.6');
define('AI_CONTENT_TOOLKIT_PLUGIN_URL', plugin_dir_url(__FILE__));
define('AI_CONTENT_TOOLKIT_PLUGIN_PATH', plugin_dir_path(__FILE__));
define('AI_CONTENT_TOOLKIT_PLUGIN_BASENAME', plugin_basename(__FILE__));

// 国际化支持已移除 - 直接使用WordPress原生翻译函数

// 加载日志管理
require_once AI_CONTENT_TOOLKIT_PLUGIN_PATH . 'includes/class-logger.php';

// 加载基础模块类
require_once AI_CONTENT_TOOLKIT_PLUGIN_PATH . 'includes/abstract-class-module-base.php';

// 加载管理页面模板系统
require_once AI_CONTENT_TOOLKIT_PLUGIN_PATH . 'includes/class-admin-page-template.php';

// 加载统一核心类
require_once AI_CONTENT_TOOLKIT_PLUGIN_PATH . 'includes/class-security-validator.php';
require_once AI_CONTENT_TOOLKIT_PLUGIN_PATH . 'includes/class-cache-manager.php';
require_once AI_CONTENT_TOOLKIT_PLUGIN_PATH . 'includes/class-database-manager.php';
require_once AI_CONTENT_TOOLKIT_PLUGIN_PATH . 'includes/class-utilities.php';

// 加载REST代理修复模块
require_once AI_CONTENT_TOOLKIT_PLUGIN_PATH . 'modules/rest-proxy-fix.php';
...
class WordPress_Toolkit {
    ...
    private function init_hooks() {
        ...
        // 插件链接
        add_filter('plugin_action_links_' . AI_CONTENT_TOOLKIT_PLUGIN_BASENAME, array($this, 'add_plugin_links'));
    }
    ...
    private function load_modules() {
        // 加载Custom Card模块（如果文件存在）
        if (file_exists(AI_CONTENT_TOOLKIT_PLUGIN_PATH . 'modules/custom-card/custom-card-module.php')) {
            require_once AI_CONTENT_TOOLKIT_PLUGIN_PATH . 'modules/custom-card/custom-card-module.php';
            $this->custom_card = Custom_Card_Module::get_instance();
        }
        
        // 加载Age Calculator模块
        require_once AI_CONTENT_TOOLKIT_PLUGIN_PATH . 'modules/age-calculator/age-calculator-module.php';
        ...
        // 加载CookieGuard模块
        require_once AI_CONTENT_TOOLKIT_PLUGIN_PATH . 'modules/cookieguard/cookieguard-module.php';
        $this->cookieguard = CookieGuard_Module::get_instance();

        // 加载Simple FriendLink模块
        require_once AI_CONTENT_TOOLKIT_PLUGIN_PATH . 'modules/simple-friendlink/simple-friendlink-module.php';
        $this->simple_friendlink = Simple_FriendLink_Module::get_instance();

        // 加载Simple FriendLink管理页面
        require_once AI_CONTENT_TOOLKIT_PLUGIN_PATH . 'modules/simple-friendlink/admin.php';
        $this->simple_friendlink_admin = new Simple_FriendLink_Admin();

        // 加载AI Settings模块 - 必须在其他需要AI功能的模块之前加载
        require_once AI_CONTENT_TOOLKIT_PLUGIN_PATH . 'modules/ai-settings/ai-settings-module.php';
        WordPress_Toolkit_AI_Settings::get_instance(); // 确保AI设置模块被实例化，加载helper函数

        // 加载Prompt Settings模块 - 必须在其他需要AI功能的模块之前加载
        require_once AI_CONTENT_TOOLKIT_PLUGIN_PATH . 'modules/prompt-settings/prompt-settings-module.php';
        WordPress_Toolkit_Prompt_Settings::get_instance(); // 确保提示词设置模块被实例化

        // 加载Auto Excerpt模块
        require_once AI_CONTENT_TOOLKIT_PLUGIN_PATH . 'modules/auto-excerpt/auto-excerpt-module.php';
        $this->auto_excerpt = Auto_Excerpt_Module::get_instance();

        // 加载Tag Optimization模块
        require_once AI_CONTENT_TOOLKIT_PLUGIN_PATH . 'modules/tag-optimization/tag-optimization-module.php';
        ...
        require_once AI_CONTENT_TOOLKIT_PLUGIN_PATH . 'modules/category-optimization/category-optimization-module.php';
        ...
        // 加载AI Category模块
        require_once AI_CONTENT_TOOLKIT_PLUGIN_PATH . 'modules/ai-category/ai-category-module.php';
        $this->ai_category = AI_Category_Module::get_instance();

        // 加载Website Optimization模块
        require_once AI_CONTENT_TOOLKIT_PLUGIN_PATH . 'modules/website-optimization/website-optimization-module.php';
    ...
    public function activate() {
        ...
        // 设置插件激活时间
        add_option('wordpress_ai_toolkit_activated_time', current_time('timestamp'));
    }
    ...
    public function add_admin_menu() {
        ...
        if (current_user_can('manage_options')) {
            add_submenu_page(
                'wordpress-ai-toolkit',
                __('网站卡片', 'wordpress-ai-toolkit'),
                __('网站卡片', 'wordpress-ai-toolkit'),
                'manage_options',
                'wordpress-ai-toolkit-cards-list',
        ...
        if (current_user_can('manage_options')) {
            add_submenu_page(
                'wordpress-ai-toolkit',
                __('友情链接', 'wordpress-ai-toolkit'),
                __('友情链接', 'wordpress-ai-toolkit'),
                'manage_options',
                'wordpress-ai-toolkit-friendlinks',
        ...
        if (current_user_can('manage_options')) {
            add_submenu_page(
                'wordpress-ai-toolkit',
                __('标签优化', 'wordpress-ai-toolkit'),
                __('标签优化', 'wordpress-ai-toolkit'),
                'manage_options',
                'wordpress-ai-toolkit-tag-optimization',
        ...
        if (current_user_can('manage_options')) {
            add_submenu_page(
                'wordpress-ai-toolkit',
                __('分类优化', 'wordpress-ai-toolkit'),
                __('分类优化', 'wordpress-ai-toolkit'),
                'manage_options',
                'wordpress-ai-toolkit-category-optimization',
        ...
        if (current_user_can('manage_options')) {
            add_submenu_page(
                'wordpress-ai-toolkit',
                __('网站优化', 'wordpress-ai-toolkit'),
                __('网站优化', 'wordpress-ai-toolkit'),
                'manage_options',
                'wordpress-ai-toolkit-website-optimization',
        ...
        add_submenu_page(
            'wordpress-ai-toolkit-settings',
            __('网站卡片设置', 'wordpress-ai-toolkit'),
            __('网站卡片', 'wordpress-ai-toolkit'),
            'manage_options',
            'wordpress-ai-toolkit-custom-card-settings',
        ...
        add_submenu_page(
            'wordpress-ai-toolkit-settings',
            __('年龄计算器设置', 'wordpress-ai-toolkit'),
            __('年龄计算器', 'wordpress-ai-toolkit'),
            'manage_options',
            'wordpress-ai-toolkit-age-calculator-settings',
        ...
        add_submenu_page(
            'wordpress-ai-toolkit-settings',
            __('Cookie同意设置', 'wordpress-ai-toolkit'),
            __('Cookie同意', 'wordpress-ai-toolkit'),
            'manage_options',
            'wordpress-ai-toolkit-cookieguard-settings',
        ...
        add_submenu_page(
            'wordpress-ai-toolkit-settings',
            __('简洁友情链接设置', 'wordpress-ai-toolkit'),
            __('简洁友情链接', 'wordpress-ai-toolkit'),
            'manage_options',
            'wordpress-ai-toolkit-simple-friendlink-settings',
        ...
        add_submenu_page(
            'wordpress-ai-toolkit-settings',
            __('文章优化设置', 'wordpress-ai-toolkit'),
            __('文章优化', 'wordpress-ai-toolkit'),
            'manage_options',
            'wordpress-ai-toolkit-auto-excerpt-settings',
        ...
        add_submenu_page(
            'wordpress-ai-toolkit-settings',
            __('AI自动分类设置', 'wordpress-ai-toolkit'),
            __('AI自动分类', 'wordpress-ai-toolkit'),
            'manage_options',
            'wordpress-ai-toolkit-ai-category-settings',
    ...
    public function admin_enqueue_scripts($hook) {
        // 只在插件相关页面加载统一样式和脚本
        if (strpos($hook, 'wordpress-ai-toolkit') !== false || strpos($hook, 'options-general') !== false) {
            // 加载统一CSS变量
            wp_enqueue_style(
                'toolkit-variables',
            ...
            wp_enqueue_style(
                'toolkit-seo-report',
                AI_CONTENT_TOOLKIT_PLUGIN_URL . 'assets/css/seo-report.css',
            ...
            wp_enqueue_script(
                'toolkit-migration',
                AI_CONTENT_TOOLKIT_PLUGIN_URL . 'assets/js/migration-helper.js',
    ...
    public function toolbox_about_page() {
        ?>
        <div class="wrap">
            <h1><?php _e('WordPress Toolkit', 'wordpress-ai-toolkit'); ?></h1>

            <div class="notice notice-info">
                <p><strong><?php _e('欢迎使用 WordPress Toolkit！', 'wordpress-ai-toolkit'); ?></strong></p>
                <p><?php _e('这是一个功能强大的WordPress综合工具包，集成了多个实用模块。', 'wordpress-ai-toolkit'); ?></p>
            </div>

            <div class="card">
                <h2><?php _e('可用模块', 'wordpress-ai-toolkit'); ?></h2>
                <p><?php _e('请从左侧菜单选择相应模块进行管理。', 'wordpress-ai-toolkit'); ?></p>

                <ul>
                    <li><strong><?php _e('网站卡片', 'wordpress-ai-toolkit'); ?></strong> - <?php _e('自动抓取网站元数据并生成美观卡片', 'wordpress-ai-toolkit'); ?></li>
                    <li><strong><?php _e('年龄计算器', 'wordpress-ai-toolkit'); ?></strong> - <?php _e('精确计算年龄，支持闰年优化', 'wordpress-ai-toolkit'); ?></li>
                    <li><strong><?php _e('物品管理', 'wordpress-ai-toolkit'); ?></strong> - <?php _e('记录和管理个人物品购买信息', 'wordpress-ai-toolkit'); ?></li>
                    <li><strong><?php _e('友情链接', 'wordpress-ai-toolkit'); ?></strong> - <?php _e('完整的友情链接管理和展示系统', 'wordpress-ai-toolkit'); ?></li>
                    <li><strong><?php _e('文章优化', 'wordpress-ai-toolkit'); ?></strong> - <?php _e('智能文章摘要生成和SEO分析', 'wordpress-ai-toolkit'); ?></li>
                    <li><strong><?php _e('分类优化', 'wordpress-ai-toolkit'); ?></strong> - <?php _e('AI智能分类描述生成', 'wordpress-ai-toolkit'); ?></li>
                                    </ul>
            </div>

            <div class="card">
                <h2><?php _e('快速链接', 'wordpress-ai-toolkit'); ?></h2>
                <p>
                    <a href="<?php echo admin_url('admin.php?page=wordpress-ai-toolkit-auto-excerpt'); ?>" class="button button-primary"><?php _e('文章优化', 'wordpress-ai-toolkit'); ?></a>
                  </p>
            </div>
        </div>
        <?php
    ...
    public function custom_card_settings_page() {
        ...
        // 验证nonce（防止CSRF攻击）
        if (isset($_POST['action']) && !wp_verify_nonce($_POST['_wpnonce'], 'wordpress_ai_toolkit_custom_card')) {
    ...
    public function age_calculator_settings_page() {
        ...
        // 验证nonce（防止CSRF攻击）
        if (isset($_POST['action']) && !wp_verify_nonce($_POST['_wpnonce'], 'wordpress_ai_toolkit_age_calculator')) {
            wp_die(__('安全验证失败', 'wordpress-ai-toolkit'));
        }
    ...
    public function time_capsule_settings_page() {
        ...
        // 验证nonce（防止CSRF攻击）
        if (isset($_POST['action']) && !wp_verify_nonce($_POST['_wpnonce'], 'wordpress_ai_toolkit_time_capsule')) {
            wp_die(__('安全验证失败', 'wordpress-ai-toolkit'));
        }
    ...
    public function cookieguard_settings_page() {
        ...
        // 验证nonce（防止CSRF攻击）
        if (isset($_POST['action']) && !wp_verify_nonce($_POST['_wpnonce'], 'wordpress_ai_toolkit_cookieguard')) {
            wp_die(__('安全验证失败', 'wordpress-ai-toolkit'));
        }
        ...
        // 获取设置
        $settings = get_option('wordpress_ai_toolkit_cookieguard_settings', array(
            'cookie_types' => array(),
            'theme' => 'light',
            'position' => 'bottom',
            'learn_more_url' => '',
        ...
        ?>
        <div class="wrap">
            <h1><?php _e('Cookie同意设置', 'wordpress-ai-toolkit'); ?></h1>

            <form method="post" action="">
                <input type="hidden" name="action" value="save_cookieguard_settings">
                <?php wp_nonce_field('wordpress_ai_toolkit_cookieguard'); ?>

                <table class="form-table">
                    <tr>
                        <th scope="row"><?php _e('Cookie类型', 'wordpress-ai-toolkit'); ?></th>
                        <td>
                            <label>
                                <input type="checkbox" name="cookie_types[]" value="necessary" checked disabled>
                                <?php _e('必要Cookie', 'wordpress-ai-toolkit'); ?> (<?php _e('始终启用', 'wordpress-ai-toolkit'); ?>)
                            </label><br>
                            <label>
                                <input type="checkbox" name="cookie_types[]" value="functional" <?php echo in_array('functional', $settings['cookie_types']) ? 'checked' : ''; ?>>
                                <?php _e('功能性Cookie', 'wordpress-ai-toolkit'); ?>
                            </label><br>
                            <label>
                                <input type="checkbox" name="cookie_types[]" value="analytics" <?php echo in_array('analytics', $settings['cookie_types']) ? 'checked' : ''; ?>>
                                <?php _e('分析性Cookie', 'wordpress-ai-toolkit'); ?>
                            </label><br>
                            <label>
                                <input type="checkbox" name="cookie_types[]" value="marketing" <?php echo in_array('marketing', $settings['cookie_types']) ? 'checked' : ''; ?>>
                                <?php _e('营销性Cookie', 'wordpress-ai-toolkit'); ?>
                            </label>
                        </td>
                    </tr>

                    <tr>
                        <th scope="row"><?php _e('主题', 'wordpress-ai-toolkit'); ?></th>
                        <td>
                            <select name="theme">
                                <option value="light" <?php selected($settings['theme'], 'light'); ?>><?php _e('浅色', 'wordpress-ai-toolkit'); ?></option>
                                <option value="dark" <?php selected($settings['theme'], 'dark'); ?>><?php _e('深色', 'wordpress-ai-toolkit'); ?></option>
                                <option value="auto" <?php selected($settings['theme'], 'auto'); ?>><?php _e('自动', 'wordpress-ai-toolkit'); ?></option>
                            </select>
                        </td>
                    </tr>

                    <tr>
                        <th scope="row"><?php _e('位置', 'wordpress-ai-toolkit'); ?></th>
                        <td>
                            <select name="position">
                                <option value="bottom" <?php selected($settings['position'], 'bottom'); ?>><?php _e('底部', 'wordpress-ai-toolkit'); ?></option>
                                <option value="top" <?php selected($settings['position'], 'top'); ?>><?php _e('顶部', 'wordpress-ai-toolkit'); ?></option>
                                <option value="center" <?php selected($settings['position'], 'center'); ?>><?php _e('居中', 'wordpress-ai-toolkit'); ?></option>
                            </select>
                        </td>
                    </tr>

                    <tr>
                        <th scope="row"><?php _e('了解更多链接', 'wordpress-ai-toolkit'); ?></th>
                        <td>
                            <input type="url" name="learn_more_url" value="<?php echo esc_url($settings['learn_more_url']); ?>" class="regular-text">
                            <p class="description"><?php _e('Cookie使用说明页面链接', 'wordpress-ai-toolkit'); ?></p>
                        </td>
                    </tr>

                    <tr>
                        <th scope="row"><?php _e('隐私政策链接', 'wordpress-ai-toolkit'); ?></th>
                        <td>
                            <input type="url" name="privacy_policy_url" value="<?php echo esc_url($settings['privacy_policy_url']); ?>" class="regular-text">
                            <p class="description"><?php _e('隐私政策页面链接', 'wordpress-ai-toolkit'); ?></p>
                        </td>
                    </tr>

                    <tr>
                        <th scope="row"><?php _e('同意有效期', 'wordpress-ai-toolkit'); ?></th>
                        <td>
                            <input type="number" name="consent_expiry_days" value="<?php echo $settings['consent_expiry_days']; ?>" min="1" max="3650" step="1">
                            <p class="description"><?php _e('用户Cookie同意记录的有效天数（默认：365天）', 'wordpress-ai-toolkit'); ?></p>
                        </td>
                    </tr>
                </table>
    ...
    private function save_cookieguard_settings() {
        ...
        update_option('wordpress_ai_toolkit_cookieguard_settings', $settings);

        // 显示成功消息
        add_settings_error('wordpress_ai_toolkit_cookieguard_settings', 'settings_saved', __('设置已保存', 'wordpress-ai-toolkit'), 'updated');
    ...
    public function simple_friendlink_settings_page() {
        ...
        ?>
        <div class="wrap">
            <h1><?php echo __('简洁友情链接设置', 'wordpress-ai-toolkit'); ?></h1>

            <div class="toolkit-settings-form">
                <h2>🔗 基本设置</h2>
                <form method="post" action="">
                    <?php wp_nonce_field('wordpress_ai_toolkit_simple_friendlink'); ?>

                    <table class="form-table">
                    <tr>
                        <th scope="row"><?php _e('用户提交', 'wordpress-ai-toolkit'); ?></th>
                        <td>
                            <label>
                                <input type="checkbox" name="allow_user_submit" value="1" <?php checked($settings['allow_user_submit']); ?>>
                                <?php _e('允许用户提交友情链接', 'wordpress-ai-toolkit'); ?>
                            </label>
                        </td>
                    </tr>

                    <tr>
                        <th scope="row"><?php _e('需要登录', 'wordpress-ai-toolkit'); ?></th>
                        <td>
                            <label>
                                <input type="checkbox" name="require_login" value="1" <?php checked($settings['require_login']); ?>>
                                <?php _e('用户必须登录才能提交', 'wordpress-ai-toolkit'); ?>
                            </label>
                        </td>
                    </tr>

                    <tr>
                        <th scope="row"><?php _e('管理员审核', 'wordpress-ai-toolkit'); ?></th>
                        <td>
                            <label>
                                <input type="checkbox" name="admin_approval" value="1" <?php checked($settings['admin_approval']); ?>>
                                <?php _e('用户提交的链接需要管理员审核', 'wordpress-ai-toolkit'); ?>
                            </label>
                        </td>
                    </tr>

                    <tr>
                        <th scope="row"><?php _e('每页显示数量', 'wordpress-ai-toolkit'); ?></th>
                        <td>
                            <input type="number" name="max_links_per_page" value="<?php echo $settings['max_links_per_page']; ?>" min="5" max="50" step="5">
                            <p class="description"><?php _e('友情链接页面每页显示的链接数量（默认：30）', 'wordpress-ai-toolkit'); ?></p>
                        </td>
                    </tr>
    ...
    public function auto_excerpt_admin_page() {
        ...
        // 验证nonce（防止CSRF攻击）
        if (isset($_POST['action']) && !wp_verify_nonce($_POST['_wpnonce'], 'wordpress_ai_toolkit_auto_excerpt')) {
            wp_die(__('安全验证失败', 'wordpress-ai-toolkit'));
        }
                ...
                <div class="postbox" style="margin-top: 15px; margin-bottom: 10px;">
                    <div class="inside" style="padding: 12px 15px;">
                        <div style="display: flex; align-items: center; gap: 30px; padding: 0; flex-wrap: wrap; justify-content: space-between;">
                            ...
                            <div>
                                <strong><?php _e('有摘要文章', 'wordpress-ai-toolkit'); ?></strong>
                                <div style="margin-top: 5px;">
                            ...
                            <div>
                                <strong><?php _e('无摘要文章数量', 'wordpress-ai-toolkit'); ?></strong>
                                <div style="margin-top: 5px;">
                            ...
                            <div>
                                <strong><?php _e('摘要覆盖率', 'wordpress-ai-toolkit'); ?></strong>
                                <div style="margin-top: 5px; display: flex; align-items: center; gap: 10px;">
                                    ...
                                    <?php if ($stats['ai_generated'] > 0): ?>
                                        <span class="badge-ai" style="background: #f0f6fc; color: #0073aa; padding: 2px 6px; border-radius: 3px; font-size: 12px; border: 1px solid #c3d9ea;">🤖 <?php echo sprintf(__('AI生成：%d篇', 'wordpress-ai-toolkit'), $stats['ai_generated']); ?></span>
                                    <?php endif; ?>
                                </div>
                            </div>
                        </div>
                    ...
                    <div class="inside" style="padding: 15px;">
                        ...
                        <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 12px; flex-wrap: wrap; gap: 15px;">
                            ...
                            <div style="display: flex; align-items: center; gap: 10px; flex-wrap: wrap;">
                                <form method="get" action="" style="display: flex; align-items: center; gap: 10px; margin: 0;">
                                    <input type="hidden" name="page" value="wordpress-ai-toolkit-auto-excerpt">
                                    <select name="status" id="excerpt-status-filter">
                                        <option value="all" <?php selected(isset($_GET['status']) ? $_GET['status'] : 'all', 'all'); ?>><?php _e('全部文章', 'wordpress-ai-toolkit'); ?></option>
                                        <option value="with_excerpt" <?php selected(isset($_GET['status']) ? $_GET['status'] : 'all', 'with_excerpt'); ?>><?php _e('有摘要文章', 'wordpress-ai-toolkit'); ?></option>
                                        <option value="without_excerpt" <?php selected(isset($_GET['status']) ? $_GET['status'] : 'all', 'without_excerpt'); ?>><?php _e('无摘要文章', 'wordpress-ai-toolkit'); ?></option>
                                    </select>
                                    ...
                                    <button type="button" id="batch-generate-excerpts" class="button button-primary">
                                        <?php _e('为无摘要文章生成摘要', 'wordpress-ai-toolkit'); ?>
                                    </button>
                                    <button type="button" id="batch-generate-tags" class="button" style="margin-left: 10px; background: #9333ea; border-color: #7c3aed; color: white;">
                                        <?php _e('批量生成标签', 'wordpress-ai-toolkit'); ?>
                                    </button>
                            ...
                            <div class="tablenav-pages" style="margin: 0;">
                                ...
                                <span class="displaying-num">
                                    <?php printf(__('共 %d 个项目', 'wordpress-ai-toolkit'), $excerpt_list['total']); ?>
                                </span>
                                <?php
                        ...
                        if (empty($excerpt_list) || !isset($excerpt_list['posts'])) {
                            ...
                            <table class="wp-list-table widefat fixed striped">
                                <thead>
                                    <tr>
                                        <th scope="col" width="35%"><?php _e('标题', 'wordpress-ai-toolkit'); ?></th>
                                        <th scope="col" width="10%"><?php _e('摘要状态', 'wordpress-ai-toolkit'); ?></th>
                                        <th scope="col" width="10%"><?php _e('摘要长度', 'wordpress-ai-toolkit'); ?></th>
                                        <th scope="col" width="10%"><?php _e('内容长度', 'wordpress-ai-toolkit'); ?></th>
                                        <th scope="col" width="15%"><?php _e('发布日期', 'wordpress-ai-toolkit'); ?></th>
                                ...
                                <tbody>
                                    <tr>
                                        <td colspan="6" style="text-align: center; padding: 40px;">
                                            ...
                                            <a href="<?php echo admin_url('admin.php?page=wordpress-ai-toolkit-auto-excerpt'); ?>" class="button button-primary">
                                                清除筛选条件
                                            </a>
                                            <?php else: ?>
                                            <div style="font-size: 16px; color: #666; margin-bottom: 20px;">
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <?php if ($post['has_excerpt']): ?>
                                            <span class="status-active"><?php _e('有摘要', 'wordpress-ai-toolkit'); ?></span>
                                            ...
                                            <?php endif; ?>
                                        <?php else: ?>
                                            <span class="status-inactive"><?php _e('无摘要', 'wordpress-ai-toolkit'); ?></span>
                                        <?php endif; ?>
                                    </td>
                                    <td><?php echo $post['excerpt_length']; ?> <?php _e('字符', 'wordpress-ai-toolkit'); ?></td>
                                    <td><?php echo $post['content_length']; ?> <?php _e('字符', 'wordpress-ai-toolkit'); ?></td>
                                    <td><?php echo $post['date']; ?></td>
                                    <td>
                                        <div class="action-buttons-container">
                                            <a href="<?php echo esc_url($post['edit_url']); ?>" class="button button-small" target="_blank" style="background: #646970; color: white; border-color: #646970; margin: 0; text-decoration: none;"><?php _e('编辑', 'wordpress-ai-toolkit'); ?></a>
                                            <a href="<?php echo esc_url($post['view_url']); ?>" class="button button-small" target="_blank" style="background: #646970; color: white; border-color: #646970; margin: 0; text-decoration: none;"><?php _e('查看', 'wordpress-ai-toolkit'); ?></a>
    ...
    public function tag_optimization_admin_page() {
        ...
        if ($this->tag_optimization) {
            ...
            echo '<div class="wrap"><h1>' . __('标签优化', 'wordpress-ai-toolkit') . '</h1>';
            echo '<div class="error"><p>' . __('标签优化模块未正确加载', 'wordpress-ai-toolkit') . '</p></div></div>';
        }
    }
    ...
    public function category_optimization_admin_page() {
        ...
        // 验证nonce（防止CSRF攻击）
        if (isset($_POST['action']) && !wp_verify_nonce($_POST['_wpnonce'], 'wordpress_ai_toolkit_category_optimization')) {
            wp_die(__('安全验证失败', 'wordpress-ai-toolkit'));
        }
        ...
        if ($this->category_optimization) {
            ...
            echo '<div class="wrap"><h1>' . __('分类优化', 'wordpress-ai-toolkit') . '</h1>';
            echo '<div class="error"><p>' . __('分类优化模块未正确加载', 'wordpress-ai-toolkit') . '</p></div></div>';
        }
    }
    ...
    public function website_optimization_admin_page() {
        ...
        // 验证nonce（防止CSRF攻击）
        if (isset($_POST['action']) && !wp_verify_nonce($_POST['_wpnonce'], 'wordpress_ai_toolkit_website_optimization')) {
            wp_die(__('安全验证失败', 'wordpress-ai-toolkit'));
        }
        ...
        if ($this->website_optimization) {
            ...
            echo '<div class="wrap"><h1>' . __('网站优化', 'wordpress-ai-toolkit') . '</h1>';
            echo '<div class="error"><p>' . __('网站优化模块未正确加载', 'wordpress-ai-toolkit') . '</p></div></div>';
        }
    }
    ...
    public function custom_cards_list_page() {
        ...
        if ($this->custom_card) {
            ...
            echo '<div class="wrap"><h1>' . __('网站卡片', 'wordpress-ai-toolkit') . '</h1>';
            echo '<div class="error"><p>' . __('网站卡片模块未正确加载', 'wordpress-ai-toolkit') . '</p></div></div>';
        }
    }
    ...
    public function time_capsule_admin_page() {
        ...
        if ($this->time_capsule) {
            ...
            echo '<div class="wrap"><h1>' . __('物品管理', 'wordpress-ai-toolkit') . '</h1>';
            echo '<div class="error"><p>' . __('物品管理模块未正确加载', 'wordpress-ai-toolkit') . '</p></div></div>';
        }
    }
    ...
    public function friendlinks_admin_page() {
        ...
        if ($this->simple_friendlink) {
            ...
            echo '<div class="wrap"><h1>' . __('友情链接', 'wordpress-ai-toolkit') . '</h1>';
            echo '<div class="error"><p>' . __('友情链接模块未正确加载', 'wordpress-ai-toolkit') . '</p></div></div>';
        }
    }
    ...
    public function toolkit_settings_main_page() {
        ...
        ?>
        <div class="wrap">
            <h1><?php _e('工具箱设置', 'wordpress-ai-toolkit'); ?></h1>

            <div class="notice notice-info">
                <p><strong><?php _e('欢迎使用 WordPress Toolkit 设置！', 'wordpress-ai-toolkit'); ?></strong></p>
                <p><?php _e('请从左侧子菜单选择具体模块进行设置。', 'wordpress-ai-toolkit'); ?></p>
            </div>

            <div class="card">
                <h2><?php _e('快速设置', 'wordpress-ai-toolkit'); ?></h2>
                <p>
                    <a href="<?php echo admin_url('admin.php?page=wordpress-ai-toolkit-custom-card-settings'); ?>" class="button"><?php _e('网站卡片', 'wordpress-ai-toolkit'); ?></a>
                    <a href="<?php echo admin_url('admin.php?page=wordpress-ai-toolkit-age-calculator-settings'); ?>" class="button"><?php _e('年龄计算器', 'wordpress-ai-toolkit'); ?></a>
                    <a href="<?php echo admin_url('admin.php?page=wordpress-ai-toolkit-cookieguard-settings'); ?>" class="button"><?php _e('Cookie同意', 'wordpress-ai-toolkit'); ?></a>
                    <a href="<?php echo admin_url('admin.php?page=wordpress-ai-toolkit-simple-friendlink-settings'); ?>" class="button"><?php _e('友情链接', 'wordpress-ai-toolkit'); ?></a>
                    <a href="<?php echo admin_url('admin.php?page=wordpress-ai-toolkit-auto-excerpt-settings'); ?>" class="button"><?php _e('文章优化', 'wordpress-ai-toolkit'); ?></a>
                    <a href="<?php echo admin_url('admin.php?page=wordpress-ai-toolkit-ai-category-settings'); ?>" class="button"><?php _e('AI自动分类', 'wordpress-ai-toolkit'); ?></a>
                </p>
            </div>

            <div class="card">
                <h2><?php _e('系统信息', 'wordpress-ai-toolkit'); ?></h2>
                <table class="widefat">
                    <tr>
                        <td><strong><?php _e('插件版本', 'wordpress-ai-toolkit'); ?></strong></td>
                        <td><?php echo AI_CONTENT_TOOLKIT_VERSION; ?></td>
                    </tr>
                    <tr>
                        <td><strong><?php _e('WordPress版本', 'wordpress-ai-toolkit'); ?></strong></td>
                        <td><?php echo get_bloginfo('version'); ?></td>
                    </tr>
                    <tr>
                        <td><strong><?php _e('PHP版本', 'wordpress-ai-toolkit'); ?></strong></td>
    ...
    public function auto_excerpt_settings_page() {
        ...
        if ($this->auto_excerpt) {
            ...
            echo '<div class="wrap"><h1>' . __('文章优化设置', 'wordpress-ai-toolkit') . '</h1>';
            echo '<div class="error"><p>' . __('文章优化模块未正确加载', 'wordpress-ai-toolkit') . '</p></div></div>';
        }
    }
    ...
    public function ai_category_settings_page() {
        ...
        if ($this->ai_category) {
            $this->ai_category->settings_page();
        } else {
            echo '<div class="wrap"><h1>' . __('AI自动分类设置', 'wordpress-ai-toolkit') . '</h1>';
            echo '<div class="error"><p>' . __('AI自动分类模块未正确加载', 'wordpress-ai-toolkit') . '</p></div></div>';
        }
    }
    ...
    public function add_plugin_links($links) {
        $settings_link = '<a href="' . admin_url('admin.php?page=wordpress-ai-toolkit') . '">设置</a>';
        $about_link = '<a href="' . admin_url('admin.php?page=wordpress-ai-toolkit-about') . '">功能说明</a>';
